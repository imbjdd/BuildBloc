...
export default async function Index({ params, searchParams }: { params: { slug: string, category: string }; searchParams: { page: string } }) {
  ...
  //const { data: hackathons } = await supabase.from("list").select()

  const db = {
    hackathon: 'list'
  }

  // use of any, it's noy good
  const element: string = (db as any)[params.category] || 'list'

  let { data: hackathons, error } = await supabase.from(element).select().ilike('name', '%'+ params.slug +'%')
